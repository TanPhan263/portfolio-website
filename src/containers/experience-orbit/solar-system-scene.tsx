...
const Planet = ({ type, onClick }: { type: PlanetType; onClick?: () => void }) => {
        ...
        >
          <div
            className={cn(
              ...
              isActive ? 'opacity-100 scale-100' : 'opacity-0 scale-50 translate-y-8'
            )}
          >
            ...
            <div className="absolute w-52 top-2 left-17 bg-black/40 backdrop-blur-xl border-l-2 border-l-white p-3 shadow-[0_0_20px_rgba(59,130,246,0.2)] pointer-events-auto transition-colors cursor-crosshair font-orbitron!">
              <p className="text-xs font-bold tracking-[0.2em] text-white mb-1 uppercase">{type}</p>
              <p className="text-white text-[9px] font-medium leading-relaxed">{description}</p>
            </div>
          </div>
        </Html>
      )}
...
const CameraRig = ({
  ...
  useFrame(state => {
    ...
    const baseR = Math.sqrt(defaultOffsetZ * defaultOffsetZ + defaultOffsetY * defaultOffsetY);
    const R = baseR * zoomLevel.current;
    const basePhi = Math.acos(defaultOffsetY / baseR);

    // Apply drag interactions safely
    const targetPhi = Math.max(0.1, Math.min(Math.PI - 0.1, basePhi + dragOffset.current.y));
    ...
    v1.copy(lookAtTarget).add(new THREE.Vector3(offsetX, finalOffsetY, finalOffsetZ));
...
const VirtualPilot = ({ targetProgress }: { targetProgress: React.MutableRefObject<number> }) => {
  useFrame(() => {
    ...
    const next = THREE.MathUtils.lerp(scrollProgress, targetProgress.current, 0.05);
    ...
    const index = Math.max(0, Math.min(Math.round(mapped), PLANET_ORDER.length - 1));
  ...
  return (
    ...
    >
      <Canvas dpr={[1, 2]} gl={{ antialias: true, alpha: false }}>
        ...
        <PerspectiveCamera makeDefault fov={fov} position={[0, 50, 200]} far={2000} />
        ...
        <pointLight
          ...
          distance={500}
        />
        <pointLight position={[0, 0, 0]} intensity={7} color="#ffffff" decay={2} distance={800} />
        <Stars radius={250} depth={60} count={5000} factor={4} saturation={0} fade speed={1} />
        ...
        {PLANET_ORDER.map((type, i) => (
          <React.Fragment key={type}>
            {PLANET_CONFIG[type].radius > 0 && (
              <OrbitPath radius={PLANET_CONFIG[type].radius} color={PLANET_CONFIG[type].color} />
            )}
