...
export const PlanetDrawer = ({ isOpen, activePlanet, onClose, children }: PlanetDrawerProps) => {
    ...
    >
      <DrawerContent className="w-full! md:w-fit! max-w-4xl z-60 dark:bg-dark/50 backdrop-blur-xl border-l border-white/10 shadow-2xl overflow-y-auto overflow-x-hidden sm:max-w-6xl! font-orbitron!">
        <div className="flex flex-col min-h-full p-0 md:p-6">
          <DrawerHeader className="flex flex-row items-center justify-between mb-4 pb-4 border-b border-white/10">
          ...
          <div className="flex-1 space-y-4 mt-4 overflow-x-hidden scrollbar-hide">{children}</div>
        </div>
      </DrawerContent>
    </Drawer>
  );
