...
export function TechnicalMarquee({
  ...
  const renderImage = (image: string, index: number) => {
    return (
      <div  key={`marquee-vertical${index}-${image}`} className="relative p-4 overflow-hidden rounded-xl border border-blue-900">
        <Image
  ...
  return (
    <div className={cn("relative flex h-96 w-full flex-row items-center justify-center gap-4 overflow-hidden [perspective:300px]", className)}>
      <div
